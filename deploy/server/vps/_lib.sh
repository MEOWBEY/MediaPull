#!/usr/bin/env bash
# Shared helpers for the DirectStream VPS scripts (install/update/uninstall).
#
...
#
# Everything here is the code install.sh and update.sh used to duplicate
# (git sync, dependency installs, yt-dlp / PO-token upgrades, client build,
# systemd templating, health check) plus the token-based git auth that stops
# a private repo from re-prompting for credentials on every clone/fetch/pull.
...
sync_systemd_unit() {
  ...
  sed "s/__PORT__/$port/; s/__MEMORY_MAX__/$mem/; s/__CPU_QUOTA__/$cpu/" \
